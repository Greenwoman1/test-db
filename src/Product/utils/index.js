...
const { Product, Variant, Topons, GroupOption, Option, GroupRule, VariantLocation, Location, SKURule, SKU, ComboItem } = require("../../index");
...
const updateOrCreateTopon = async (toponData, variantId, t) => {
	const { toponId, quantity } = toponData;

	try {
		let topon;

		if (toponId) {
			topon = await Topons.findByPk(toponId);
		}
