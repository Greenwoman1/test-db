...
const { Variant, Topon, Option, GroupRule, Product, Location, Image, VariantTopons, VariantLocation, GroupOptions, PriceHistory, Combo, ComboItems, SKU, Category, Ingredient, IngredientLocation, ToponLocation, VariantIngredient, VariantSKURule, LinkedVariant, IngredientSKURule, GroupTopon, GroupToponsMid, ToponSKURule, UserLocation, User } = require('../index');
...
const { findAndCountAll } = require('./Product.js');


const list = async (req, res, io) => {
  try {
    const { name, description, categoryId, page = 1, limit = 5, locationId, type } = req.query;
          ...
          include: [
            {
              ...
              include: [
                ...
                { model: GroupOptions, as: 'GO', include: [{ model: Option }] }
              ]
            }
          ]
        },
...
const getMenu = async (req, res) => {
  ...
  try {
    ...
    const userLocation = await UserLocation.findOne({ where: { UserId: userId } });
    if (!userLocation) {
        ...
        include: [{
          ...
          include : [
            ...
            include: [{
              ...
              attributes: ['id', 'min', 'max', 'disabled', 'default'],
              include: [{
                model: ToponLocation,
                // attributes: ['id'],
                as: 'TLoc',
...
module.exports = { list, getProductById, getProductVariants, getProductVariantLocation, getProductsAtLocation, createProduct, createBasicProduct, getProductDetails, getMenu };
